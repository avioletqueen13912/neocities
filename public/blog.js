...
[ "posts/2026-02-02-a-new-month.html", 
  "vibes-based behaviors..." ],
[ "posts/2025-06-06-a-possible-return.html", 
  "it's been a while since i was active here." ],
...
let blogHeaderHTML = '<ul> <li><a href="' + relativePath + '/index.html">Home</a></li>' + 
...
let blogFooterHTML = "<hr><p>" + blogName + " is written by <a href='" + authorLink + "'>" + authorName + "</a>, built with <a href='https://zonelets.net/'>Zonelets</a>, and hosted by <a href='https://neocities.org/'>Neocities!</a></p>";
...
function formatPostLink(i) {
  let postTitle_i = "";
  if ( postsArray[i].length > 2 ) {
    postTitle_i = decodeURI(postsArray[i][2]);
  } else {
	if (  postDateFormat.test ( postsArray[i][0].slice( 6,17 ) ) ) {
...
function formatSummaryLink(i) {
  ...
  if (  postDateFormat.test ( postsArray[i][0].slice( 6,17 ) ) ) {
    let returnStr = '<section class="postsummary"><p>';
    returnStr += '<div class="postsummarytitle">' + postTitle_i + '</div>';
    returnStr += '<div class="postsummarydate">' + postsArray[i][0].slice(6,16) + '</div>';
    returnStr += '<div class="postsummaryblog">' + postsArray[i][1] + '</div>';
    returnStr += '<div class="readmorebutton"><div class="readmorelink"><a href="' + relativePath + '/'+ postsArray[i][0] +'">\u00BB read more</a></div></div>';
    returnStr += '</p></section><br />';
    return returnStr;
  } else {
    let returnStr = '<section class="postsummary"><p>';
    returnStr += '<div class="postsummarytitle">' + postTitle_i + '</div>';
...
let recentPostListHTML = "<br>";
let numberOfRecentPosts = Math.min( recentPostsCutoff, postsArray.length );
for ( let i = 0; i < numberOfRecentPosts; i++ ) {
  recentPostListHTML += formatSummaryLink(i);
}
...
if ( postsArray.length > recentPostsCutoff ) {
  recentPostListHTML += '<div class="moreposts"><a href=' + relativePath + '/posts/archive.html>\u00BB archive</a></div>';
}
...
if ( postsArray.length < 2 ) {
  nextprevHTML = '<a href="' + relativePath + '/blog.html">blog home</a>';
} else if ( currentIndex === 0 ) {
  prevlink = postsArray[currentIndex + 1][0];
  nextprevHTML = '<a href="' + relativePath + '/blog.html">blog home</a> | <a href="'+ relativePath + '/' + prevlink +'">previous post \u00BB</a>';
} else if ( currentIndex === postsArray.length - 1 ) {
  nextlink = postsArray[currentIndex - 1][0];
  nextprevHTML = '<a href="' + relativePath + '/' + nextlink +'">\u00AB next post</a> | <a href="' + relativePath + '/blog.html">blog home</a>';
} else if ( 0 < currentIndex && currentIndex < postsArray.length - 1 ) {
  nextlink = postsArray[currentIndex - 1][0];
  prevlink = postsArray[currentIndex + 1][0];
  nextprevHTML = '<a href="' + relativePath + '/'+ nextlink +'">\u00AB next post</a> | <a href="' + relativePath + '/blog.html">blog home</a> | <a href="' + relativePath + '/'+ prevlink +'">previous post \u00BB</a>';
}
...
if (document.getElementById("header")) {
  document.getElementById("header").innerHTML = blogHeaderHTML;
}
...
if (document.getElementById("footer")) {
  document.getElementById("footer").innerHTML = blogFooterHTML;
}
